...
const TagsManagement = ({ onTagChange, onStatsUpdate }) => {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      setDeletingId(tagToDelete._id);
      await AdminAPI.deleteTag(tagToDelete._id);
      setTags(tags.filter((t) => t._id !== tagToDelete._id));
      
      // Notify dashboard of tag count change
      if (onTagChange) onTagChange(-1);
  ...
  const handleCreate = async (e) => {
    ...
    try {
      setCreating(true);
      const res = await AdminAPI.createTag({ name: newTag.name });
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-8">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <h2 className="text-3xl font-bold text-gray-800 mb-2">Manage Tags</h2>
              <p className="text-gray-600">Create and organize tags for better content categorization</p>
            </div>
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className="bg-green-500 hover:bg-green-600 text-white px-6 py-3 rounded-lg font-medium transition-all duration-200 ease-in-out transform hover:scale-105 active:scale-95 shadow-sm hover:shadow-md flex items-center space-x-2"
              >
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {showCreateForm && (
          <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-6 mb-6 animate-fadeIn">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-xl font-semibold text-gray-800">Create New Tag</h3>
              <button
            ...
            <form onSubmit={handleCreate} className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={handleInputChange}
                  rows={3}
                  className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-all duration-200 outline-none resize-none"
              ...
              <div className="flex justify-end space-x-3 pt-2">
                ...
                <button
                  ...
                  className={`
                    ...
                    ${creating || !newTag.name.trim()
                      ? 'bg-green-400 cursor-not-allowed'
                      : 'bg-green-500 hover:bg-green-600 transform hover:scale-105 active:scale-95'
                    }
                  `}
                >
                  {creating ? (
            ...
            {tags.length === 0 && !showCreateForm ? (
              ...
              <div className="text-center py-12">
                ...
                <p className="text-gray-500 max-w-sm mx-auto mb-6">
                  Start by creating tags to help organize and categorize your content.
                </p>
                <button
                  onClick={() => setShowCreateForm(true)}
                  className="bg-green-500 hover:bg-green-600 text-white px-6 py-3 rounded-lg font-medium transition-all duration-200 ease-in-out transform hover:scale-105 active:scale-95 shadow-sm hover:shadow-md flex items-center space-x-2 mx-auto"
                >
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
                  </svg>
                  ...
                  >
                    <div className="flex items-center space-x-4">
                      ...
                      <div>
                        ...
                        {tag.description && (
                          <p className="text-gray-500 text-sm mt-1">{tag.description}</p>
                        )}
                      </div>
                    </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50 animate-fadeIn">
          <div className="bg-white rounded-xl shadow-2xl max-w-md w-full p-6 transform animate-scaleIn">
            <div className="flex items-center space-x-3 mb-4">
              ...
              <h3 className="text-lg font-semibold text-gray-800">Delete Tag</h3>
            </div>
            
            <p className="text-gray-600 mb-2">Are you sure you want to delete this tag?</p>
            ...
            <div className="bg-gray-50 rounded-lg p-4 mb-6">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <p className="font-semibold text-gray-800">{tagToDelete.name}</p>
                  {tagToDelete.description && (
                    <p className="text-gray-500 text-sm mt-1">{tagToDelete.description}</p>
                  )}
                </div>
              </div>
            </div>
